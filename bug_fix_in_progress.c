...
node *getnode() {
  ...
  scanf("%19s", new1->name); // Limit input to avoid buffer overflow
  new1->next = NULL;
  return new1;
}
...
void guess_number() {
  ...
  while (x != 0) {
    ...
    if (scanf("%d", &b) != 1 || b < 1 || b > 3) { // Validate input
...
void cowsandbulls() {
  ...
  while (1) {
    ...
    printf("\nEnter\n<0> to QUIT\n<1> to REPLAY\nEnter: ");
    if (scanf("%d", &quit) != 1) { // Validate input
      printf("Invalid input. Exiting the game.\n");
      clear_input_buffer();
      break;
    }
...
void rokpapciss() {
  ...
  while (ct <= 5) {
    ...
    if (n == 1) {
      ...
      if (scanf("%d", &ch) != 1 || ch < 0 || ch > 2) { // Validate input
...
int main() {
  int ch = -1; // Initialize to avoid undefined behavior
  while (ch != 4) {
    system("cls");
    system("color 03");
